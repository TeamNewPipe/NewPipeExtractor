...
public class InvidiousStreamExtractor extends StreamExtractor {
    ...
    public List<Frameset> getFrames() throws ExtractionException {
        final JsonArray storyboards = json.getArray("storyboards");
        List<Frameset> frames = new ArrayList<>(storyboards.size());
        for (Object o : storyboards) {
            final JsonObject storyboard = (JsonObject) o;
            int durationPerFrames = storyboard.getInt("interval");
            int width = storyboard.getInt("width");
            int height = storyboard.getInt("height");
            int totalCount = storyboard.getInt("count");
            int framesPerPageX = storyboard.getInt("storyboardWidth");
            int framesPerPageY = storyboard.getInt("storyboardHeight");
            int storyBoardCount = storyboard.getInt("storyboardCount");
            List<String> urls = new ArrayList<>(storyBoardCount);
            String url = storyboard.getString("templateUrl");
            for (int i = 0; i < storyBoardCount; i++) {
                urls.add(url.replace("M$M", "M" + i));
            }
            if (durationPerFrames != 0) {
                frames.add(new Frameset(urls, width, height, totalCount, durationPerFrames, framesPerPageX, framesPerPageY));
            }
        }
        return frames;
    }
